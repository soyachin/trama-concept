...
import { BG, FG, NODE_VISUALS, DEFAULT_VISUAL, BAYER } from '../config/visuals'
...
export function TramaGraph() {
  ...
  useEffect(() => { setPanelRef.current = setPanelNode }, [])
  useEffect(() => { searchRef.current = searchVal }, [searchVal])

  const graphData = useRef(processJSONLD(TRAMA_GRAPH)).current

  const getConns = useCallback((id: string) =>
    graphData.edges
      .filter(e => e.source === id || e.target === id)
      .map(e => ({
        predicate: e.predicate,
        id:    e.source === id ? e.target : e.source,
        label: graphData.nodes.find(n => n.id === (e.source === id ? e.target : e.source))?.label ?? '',
      })),
    [graphData]
  )
    ...
    const instance = new p5((p: p5) => {
      ...
      p.draw = () => {
        ...
        for (const e of edges) {
          ...
          if (!src || !tgt) continue
          const active = hovId === e.source || hovId === e.target || selId === e.source || selId === e.target
          const mSrc   = !q || src.label.toLowerCase().includes(q) || src.tags.some(t => t.includes(q))
          const mTgt   = !q || tgt.label.toLowerCase().includes(q) || tgt.tags.some(t => t.includes(q))
          let a = 0.11
          if (active)              a = 0.82
        ...
        for (const n of nodes) {
          ...
          const match = !q || n.label.toLowerCase().includes(q) || n.tags.some(t => t.toLowerCase().includes(q)) || n.description.toLowerCase().includes(q)
          let a = 0.92
      ...
      function renderIntro(ctx: CanvasRenderingContext2D, w: number, h: number) {
        if (intro === 'showing') {
          ...
          ctx.fillText('"Trama es el mapa de lo que tu universidad ya sabe,', w / 2, h / 2 - 24)
          ...
          ctx.fillText('Explora. Cada nodo es una puerta. Cada arista, una conversación pendiente.', w / 2, h / 2 + 50)
          ctx.font = `10px 'Space Mono', monospace`
          ctx.fillStyle = `rgba(200,117,58,0.65)`
          ctx.fillText('[ click para comenzar ]', w / 2, h / 2 + 84)
        } else if (intro === 'dissolving') {
          dissolve += 0.022
          if (dissolve >= 1) { intro = 'done'; return }
          const TILE = 7
      ...
      function toGraph(mx: number, my: number): [number, number] {
        return [(mx - panX - p.width / 2) / zoom + p.width / 2, (my - panY - p.height / 2) / zoom + p.height / 2]
      }
      ...
      p.mousePressed = () => {
        if (intro !== 'done') { intro = 'dissolving'; return }
        const n = hitTest(p.mouseX, p.mouseY)
        if (n) {
          dragNode = n
          dragging = false
        } else {
          dragging = false; dStartX = p.mouseX; dStartY = p.mouseY
        }
      }

      p.mouseReleased = () => {
        if (intro !== 'done') return
        if (dragNode) { simulation?.alphaTarget(0); dragNode = null }
        if (!dragging) {
          const n = hitTest(p.mouseX, p.mouseY)
          if (n) {
            if (selId === n.id) { selId = null; setPanelRef.current(null) }
            else                { selId = n.id; setPanelRef.current(n)   }
          } else { selId = null; setPanelRef.current(null) }
        }
        dragging = false
      }
      ...
      p.mouseDragged = () => {
        ...
        if (dragNode) {
          ...
          if (!dragging && Math.hypot(p.mouseX - dStartX, p.mouseY - dStartY) > 5) dragging = true
          if (dragging) { panX += p.movedX; panY += p.movedY }
        }
      }
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: 'var(--color-bg)', overflow: 'hidden', position: 'relative' }}>
      ...
      <InfoPanel node={panelNode} onClose={() => setPanelNode(null)} getConns={getConns} />
      <SearchBar value={searchVal} onChange={setSearchVal} offset={panelNode !== null} />
      <EdgeLegend />

      <div style={{ position: 'absolute', top: 14, right: 14, zIndex: 20, fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'color-mix(in srgb, var(--color-fg) 20%, transparent)', letterSpacing: '0.1em' }}>
        trama · seed:42
      </div>
    </div>
  )
